...
class FindEntityEnvironment extends React.Component {
    ...
    handleCollectionItemCheckboxChange = (key, checked) => {
        const {collectionCheckboxGroupValue} = this.state;
        let newValue = checked ? [...collectionCheckboxGroupValue, key] : collectionCheckboxGroupValue.filter(item => item !== key)
        this.setState({collectionCheckboxGroupValue: newValue})
    }
    ...
    render() {
        ...
        const {allCollection = [], collectionPopoverVisible,
